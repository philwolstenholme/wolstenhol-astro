  ...
  return (
    ...
    >
      ...
      {keyHint && (
        <div
          aria-hidden="true"
          class="pointer-fine:hidden absolute top-2 right-2 z-10 flex h-5 w-5 items-center justify-center rounded border border-yellow-300/60 text-xs font-bold text-yellow-300 opacity-0 transition-opacity duration-150 group-[.keyboard-active]/player:opacity-100"
        >
          {keyHint}
        </div>
      )}
...
export function SpotifyMusicPlayer({ artists }: Props) {
  ...
  return (
    ...
    >
      ...
      <p class="pointer-fine:hidden mt-2 pb-0.5 text-xs opacity-90">
