import { defineAlpineComponent } from '../../utils/alpine';

export const PwHeader = defineAlpineComponent(() => ({
  activeSection: null as string | null | undefined,
  init() {
    ...
    const observer = new IntersectionObserver(
      entries => {
        ...
        if (headingsInView.length > 0) {
          const sectionId = (this.activeSection = (headingsInView[0] as HTMLElement).dataset.section);
